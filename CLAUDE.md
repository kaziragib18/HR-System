...
Last updated: 2026-07-16 — the 4-milestone security/completeness roadmap below is fully shipped, plus the additions listed under "Shipped after Milestones 1-4". Keep this section current — see "Keeping this section current" below.
...
- Test coverage has grown past initial scaffolding: Vitest in `packages/utils` (date/tax/leave-calculator/attendance, 38 tests across 4 files) and `apps/api` (78 tests across 11 files — Prisma-mocked service tests for attendance including adjustments/manual-entry/self-check-in/listAttendance, leave, announcements, employees role-change, and approver-resolution, plus supertest regression suites for tenant-isolation and approvals-history). `npx pnpm@9 test` runs all of it.
...
- Several `apps/api` modules still have no tests (payroll, salary, departments, documents, company, notifications, dashboard, holidays, job-grades, auth) and all of `apps/web` remains untested.
...
`createNotification` (`apps/api/src/services/notification.service.ts`) is fire-and-forget: failures are logged and swallowed, never surfaced to the caller, so a broken notification insert never blocks the underlying action (leave application, approval, etc.) from succeeding. Every leave-lifecycle notification targets the single resolved reporting-chain approver/applicant (`resolveTeamApprover` — see RBAC above), not a role-wide broadcast; `SUPER_ADMIN` will not see routine leave-request notifications unless they happen to be the resolved approver. The one exception — mirroring the same fallback already used in `attendance.service.ts`'s `requestAdjustment` — is `applyLeave`'s auto-approve path: if `resolveTeamApprover` returns nothing (e.g. a department has no head assigned yet), the leave still auto-approves, and every `SUPER_ADMIN` in the office gets notified so it doesn't happen completely unnoticed. There is no admin-wide "see every notification" inbox anywhere — `GET /notifications` is always scoped to the caller's own `employeeId`, `SUPER_ADMIN` included.
